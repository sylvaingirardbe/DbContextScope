...
using Agidens.DbContextScope.Enums;
			...
			foreach (var contextInCurrentScope in _dbContexts.InitializedDbContexts.Values)
            {
                ...
                foreach (var toRefresh in entities)
                {
                    ...
                    if (stateInCurrentScope != null)
                    {
                        ...
                        var keyValue = stateInCurrentScope
                            .Entity
                            .GetType()
                        ...
                        var stateInParentScope = correspondingParentContext
                            ...
                            .Entries()
                            .SingleOrDefault(e => 
                                e.Entity.GetType() == stateInCurrentScope.Entity.GetType() && 
                                e.Entity.GetType().GetProperty(key.Name).GetValue(e.Entity).Equals(keyValue)
                            );
                        if (stateInParentScope != null)
                        {
                            ...
                            {
                                correspondingParentContext.Entry(stateInParentScope).Reload();
                            }
                        }
                    }
                }
                ...
                foreach (var toRefresh in entities)
                {
                    ...
                    if (stateInCurrentScope != null)
                    {
                        ...
                        if (stateInParentScope != null)
                        {
                            ...
                            if (stateInParentScope.State == EntityState.Unchanged)
                            {
                                await correspondingParentContext.Entry(stateInParentScope).ReloadAsync();
                            }
                        }
                    }
                }
